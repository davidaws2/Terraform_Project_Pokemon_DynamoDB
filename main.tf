...
resource "aws_instance" "pokemon_server" {
  ...
  user_data = <<-EOF
                ...
                echo "python3 /home/ec2-user/pokemon-collector/pokemon_collector.py" >> /home/ec2-user/.bashrc
                EOF
}
...
output "key_instructions" {
  value = "A new private key has been generated and saved as 'pokemon_key.pem'. Run 'chmod 400 pokemon_key.pem' to set correct permissions, then use 'ssh -i pokemon_key.pem ec2-user@${aws_instance.pokemon_server.public_ip}' to connect to the instance."
}
